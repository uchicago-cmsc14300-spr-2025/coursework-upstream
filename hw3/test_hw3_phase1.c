...
Test(hw3_board_setup, board_setup01)
{
  piece *board = board_setup();
  cr_assert(board[1]==WN);
  free(board);
}

Test(hw3_board_setup, board_setup02)
{
  piece *board = board_setup();
  cr_assert(board[62]==BN);
  free(board);
}
...
Test(hw3_board_get, board_get01)
{
  piece *board = board_setup();
  cr_assert(board_get(board,'B',1)==WN);
  free(board);
}

Test(hw3_board_get, board_get02)
{
  piece *board = board_setup();
  cr_assert(board_get(board,'D',8)==BQ);
  free(board);
}
